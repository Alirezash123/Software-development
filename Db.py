...
def login_required(fn):
    ...
    def wrapper(*args, **kwargs):
        ...
        user_id = int(get_jwt_identity())  # identity الان string است
...
def login():
    ...
    token = create_access_token(
        identity=str(user.id),            # حتماً string
        additional_claims={"role": user.role}  # نقش
    )
    # --------------------------------------
...
def doctor_comments():
    ...
    return jsonify([
        {'content': c.content, 'date': c.created_at.isoformat()} for c in comments
    ])
